...
/*=============================================================================
	Character.cpp: AEcoscapeCharacter implementation
=============================================================================*/

#include "Character/Animals/EcoscapeCharacter.h"
...
FName AEcoscapeCharacter::MeshComponentName(TEXT("CharacterMesh0"));
FName AEcoscapeCharacter::CharacterMovementComponentName(TEXT("CharMoveComp"));

AEcoscapeCharacter::AEcoscapeCharacter(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
	...
	// CapsuleComponent = CreateDefaultSubobject<UCapsuleComponent>(AEcoscapeCharacter::CapsuleComponentName);
	...
	Mesh = CreateOptionalDefaultSubobject<USkeletalMeshComponent>(AEcoscapeCharacter::MeshComponentName);
	if (Mesh)
	{
	...
	CharacterMovement = CreateDefaultSubobject<UCharacterMovementComponent>(AEcoscapeCharacter::CharacterMovementComponentName);

	BaseRotationOffset = FQuat::Identity;
}

void AEcoscapeCharacter::PostInitializeComponents()
{
...
void AEcoscapeCharacter::BeginPlay()
{
	Super::BeginPlay();
}


void AEcoscapeCharacter::CacheInitialMeshOffset(FVector MeshRelativeLocation, FRotator MeshRelativeRotation)
{
	...
	{
		logOrEnsureNanError(TEXT("AEcoscapeCharacter::PostInitializeComponents detected NaN in BaseRotationOffset! (%s)"), *BaseRotationOffset.ToString());
	}

	const FRotator LocalRotation = Mesh->GetRelativeRotation();
	if (LocalRotation.ContainsNaN())
	{
		logOrEnsureNanError(TEXT("AEcoscapeCharacter::PostInitializeComponents detected NaN in Mesh->RelativeRotation! (%s)"), *LocalRotation.ToString());
	}
#endif
}


UPawnMovementComponent* AEcoscapeCharacter::GetMovementComponent() const
{
	return CharacterMovement;
}


void AEcoscapeCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	check(PlayerInputComponent);
}


void AEcoscapeCharacter::GetSimpleCollisionCylinder(float& CollisionRadius, float& CollisionHalfHeight) const
{
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	if (IsTemplate())
	{
		UE_LOG(LogCharacter, Log, TEXT("WARNING AEcoscapeCharacter::GetSimpleCollisionCylinder : Called on default object '%s'. Will likely return zero size. Consider using GetDefaultHalfHeight() instead."), *this->GetPathName());
	}
#endif
...
void AEcoscapeCharacter::UpdateNavigationRelevance()
{
	if (Mesh)
	{
		Mesh->SetCanEverAffectNavigation(bCanAffectNavigationGeneration);
	}
}

float AEcoscapeCharacter::GetDefaultHalfHeight() const
{
		return Super::GetDefaultHalfHeight();
}

UActorComponent* AEcoscapeCharacter::FindComponentByClass(const TSubclassOf<UActorComponent> ComponentClass) const
{
...
void AEcoscapeCharacter::OnWalkingOffLedge_Implementation(const FVector& PreviousFloorImpactNormal, const FVector& PreviousFloorContactNormal, const FVector& PreviousLocation, float TimeDelta)
{
}

void AEcoscapeCharacter::NotifyJumpApex()
{
...
void AEcoscapeCharacter::Landed(const FHitResult& Hit)
{
	OnLanded(Hit);
...
bool AEcoscapeCharacter::CanJump() const
{
	return CanJumpInternal();
}

bool AEcoscapeCharacter::CanJumpInternal_Implementation() const
{
	return !bIsCrouched && JumpIsAllowedInternal();
}

bool AEcoscapeCharacter::JumpIsAllowedInternal() const
{
...
void AEcoscapeCharacter::ResetJumpState()
{
	bPressedJump = false;
	bWasJumping = false;
	JumpKeyHoldTime = 0.0f;
...
void AEcoscapeCharacter::OnJumped_Implementation()
{
}

bool AEcoscapeCharacter::IsJumpProvidingForce() const
{
...
void AEcoscapeCharacter::RecalculateBaseEyeHeight()
{
	if (!bIsCrouched)
	{
...
void AEcoscapeCharacter::OnRep_IsCrouched()
{
	if (CharacterMovement)
	{
		if (bIsCrouched)
...
void AEcoscapeCharacter::SetReplicateMovement(bool bInReplicateMovement)
{
...
bool AEcoscapeCharacter::CanCrouch() const
{
...
void AEcoscapeCharacter::Crouch(bool bClientSimulation)
{
	if (CharacterMovement)
	{
		if (CanCrouch())
...
void AEcoscapeCharacter::UnCrouch(bool bClientSimulation)
{
	if (CharacterMovement)
	{
		CharacterMovement->bWantsToCrouch = false;
...
void AEcoscapeCharacter::OnEndCrouch( float HeightAdjust, float ScaledHeightAdjust )
{
	RecalculateBaseEyeHeight();

	const AEcoscapeCharacter* DefaultChar = GetDefault<AEcoscapeCharacter>(GetClass());
...
void AEcoscapeCharacter::OnStartCrouch( float HeightAdjust, float ScaledHeightAdjust )
{
	RecalculateBaseEyeHeight();
...
void AEcoscapeCharacter::RecalculateCrouchedEyeHeight()
{
...
void AEcoscapeCharacter::ApplyDamageMomentum(float DamageTaken, FDamageEvent const& DamageEvent, APawn* PawnInstigator, AActor* DamageCauser)
{
...
void AEcoscapeCharacter::ClearCrossLevelReferences()
{
...
namespace EcoscapeMovementBaseUtility
{
	...
	{
		if (NewBase && EcoscapeMovementBaseUtility::UseRelativeLocation(NewBase))
		{
	...
	{
		if (OldBase && EcoscapeMovementBaseUtility::UseRelativeLocation(OldBase))
		{
	...
	FVector GetMovementBaseVelocity(const UPrimitiveComponent* MovementBase, const FName BoneName)
	{
		...
		if (EcoscapeMovementBaseUtility::IsDynamicBase(MovementBase))
		{
		...
		if (EcoscapeMovementBaseUtility::IsDynamicBase(MovementBase))
		{
			if (const FBodyInstance* BodyInstance = MovementBase->GetBodyInstance(BoneName))
			{
				...
				if (!BaseAngVelInRad.IsNearlyZero())
				{
					...
					FQuat BaseRotation;
					if (EcoscapeMovementBaseUtility::GetMovementBaseTransform(MovementBase, BoneName, BaseLocation, BaseRotation))
					{
...
void AEcoscapeCharacter::SetBase( UPrimitiveComponent* NewBaseComponent, const FName InBoneName, bool bNotifyPawn )
{
	...
	if (bBaseChanged || bBoneChanged)
	{
		...
		if (CharacterMovement)
		{
			const bool bBaseIsSimulating = EcoscapeMovementBaseUtility::IsSimulatedBase(NewBaseComponent);
			if (bBaseChanged)
			{
				EcoscapeMovementBaseUtility::RemoveTickDependency(CharacterMovement->PrimaryComponentTick, OldBase);
				// We use a special post physics function if simulating, otherwise add normal tick prereqs.
				if (!bBaseIsSimulating)
				{
					EcoscapeMovementBaseUtility::AddTickDependency(CharacterMovement->PrimaryComponentTick, NewBaseComponent);
				}
			}
...
void AEcoscapeCharacter::SaveRelativeBasedMovement(const FVector& NewRelativeLocation, const FRotator& NewRotation, bool bRelativeRotation)
{
...
FVector AEcoscapeCharacter::GetNavAgentLocation() const
{
	...
	{
		AgentLocation = GetActorLocation() - FVector(0, 0, UEcoscapeStatics::GetZUnderOrigin(this));
	}

	return AgentLocation;
}

void AEcoscapeCharacter::TurnOff()
{
	if (CharacterMovement != nullptr)
	{
...
void AEcoscapeCharacter::Restart()
{
	Super::Restart();
...
void AEcoscapeCharacter::PawnClientRestart()
{
	if (CharacterMovement != nullptr)
	{
		CharacterMovement->StopMovementImmediately();
...
void AEcoscapeCharacter::PossessedBy(AController* NewController)
{
...
void AEcoscapeCharacter::UnPossessed()
{
	Super::UnPossessed();
...
void AEcoscapeCharacter::TornOff()
{
	Super::TornOff();
...
void AEcoscapeCharacter::NotifyActorBeginOverlap(AActor* OtherActor)
{
	NumActorOverlapEventsCounter++;
	Super::NotifyActorBeginOverlap(OtherActor);
}

void AEcoscapeCharacter::NotifyActorEndOverlap(AActor* OtherActor)
{
	NumActorOverlapEventsCounter++;
	Super::NotifyActorEndOverlap(OtherActor);
}

void AEcoscapeCharacter::BaseChange()
{
...
void AEcoscapeCharacter::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
...
void AEcoscapeCharacter::LaunchCharacter(FVector LaunchVelocity, bool bXYOverride, bool bZOverride)
{
	UE_LOG(LogCharacter, Verbose, TEXT("AEcoscapeCharacter::LaunchCharacter '%s' (%f,%f,%f)"), *GetName(), LaunchVelocity.X, LaunchVelocity.Y, LaunchVelocity.Z);
...
void AEcoscapeCharacter::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PrevCustomMode)
{
...
bool AEcoscapeCharacter::ShouldNotifyLanded(const FHitResult& Hit)
{
...
void AEcoscapeCharacter::Jump()
{
	bPressedJump = true;
	JumpKeyHoldTime = 0.0f;
}

void AEcoscapeCharacter::StopJumping()
{
	bPressedJump = false;
	ResetJumpState();
}

void AEcoscapeCharacter::CheckJumpInput(float DeltaTime)
{
...
void AEcoscapeCharacter::ClearJumpInput(float DeltaTime)
{
	if (bPressedJump)
	{
...
float AEcoscapeCharacter::GetJumpMaxHoldTime() const
{
...
void AEcoscapeCharacter::PreNetReceive()
{
	SavedMovementMode = ReplicatedMovementMode;
	Super::PreNetReceive();
}

void AEcoscapeCharacter::PostNetReceive()
{
...
void AEcoscapeCharacter::OnRep_ReplicatedBasedMovement()
{	
	if (GetLocalRole() != ROLE_SimulatedProxy)
	{
		return;
	...
	if (ReplicatedBasedMovement.HasRelativeLocation())
	{
		...
		EcoscapeMovementBaseUtility::GetMovementBaseTransform(ReplicatedBasedMovement.MovementBase, ReplicatedBasedMovement.BoneName, CharacterMovement->OldBaseLocation, CharacterMovement->OldBaseQuat);
...
void AEcoscapeCharacter::OnRep_ReplicatedMovement()
{
...
void AEcoscapeCharacter::OnRep_ReplayLastTransformUpdateTimeStamp()
{
...
FAnimMontageInstance * AEcoscapeCharacter::GetRootMotionAnimMontageInstance() const
{
	return (Mesh && Mesh->GetAnimInstance()) ? Mesh->GetAnimInstance()->GetRootMotionMontageInstance() : nullptr;
}

void AEcoscapeCharacter::OnRep_RootMotion()
{
	if (CharacterMovement && (CharacterMovement->NetworkSmoothingMode == ENetworkSmoothingMode::Replay))
	{
		return;
	...
	if (GetLocalRole() == ROLE_SimulatedProxy)
	{
		...
		UE_LOG(LogRootMotion, Log,  TEXT("AEcoscapeCharacter::OnRep_RootMotion"));
		...
		if( RepRootMotion.bIsActive )
		{
			...
			FEcoscapeSimulatedRootMotionReplicatedMove& NewMove = RootMotionRepMoves.Last();
...
void AEcoscapeCharacter::SimulatedRootMotionPositionFixup(float DeltaSeconds)
{
	...
	if( ClientMontageInstance && CharacterMovement && Mesh )
	{
		...
		if( MoveIndex != INDEX_NONE )
		{
			...
			const FEcoscapeSimulatedRootMotionReplicatedMove& RootMotionRepMove = RootMotionRepMoves[MoveIndex];
...
int32 AEcoscapeCharacter::FindRootMotionRepMove(const FAnimMontageInstance& ClientMontageInstance) const
{
	...
	UE_LOG(LogRootMotion, Log,  TEXT("\tAEcoscapeCharacter::FindRootMotionRepMove FoundIndex: %d, NumSavedMoves: %d"), FoundIndex, RootMotionRepMoves.Num());
	return FoundIndex;
}

bool AEcoscapeCharacter::CanUseRootMotionRepMove(const FEcoscapeSimulatedRootMotionReplicatedMove& RootMotionRepMove, const FAnimMontageInstance& ClientMontageInstance) const
{
	...
	if( GetWorld()->TimeSince(RootMotionRepMove.Time) <= 0.5f )
	{
		...
		if( RootMotionRepMove.RootMotion.AnimMontage && (RootMotionRepMove.RootMotion.AnimMontage == ClientMontageInstance.Montage) )
		{
			...
			if( CurrentSectionIndex != INDEX_NONE )
			{
				...
				UE_LOG(LogRootMotion, Log,  TEXT("\t\tAEcoscapeCharacter::CanUseRootMotionRepMove ServerPosition: %.3f, ClientPosition: %.3f, DeltaPosition: %.3f, bSameSections: %d, bHasLooped: %d, bServerAheadOfClient: %d"), 
...
bool AEcoscapeCharacter::RestoreReplicatedMove(const FEcoscapeSimulatedRootMotionReplicatedMove& RootMotionRepMove)
{
	...
	{
		bool bSuccess = false;
		if( EcoscapeMovementBaseUtility::UseRelativeLocation(ServerBase) )
		{
			FVector BaseLocation;
			FQuat BaseRotation;
			EcoscapeMovementBaseUtility::GetMovementBaseTransform(ServerBase, ServerBaseBoneName, BaseLocation, BaseRotation);
...
void AEcoscapeCharacter::OnUpdateSimulatedPosition(const FVector& OldLocation, const FQuat& OldRotation)
{
...
void AEcoscapeCharacter::PostNetReceiveLocationAndRotation()
{
	if(GetLocalRole() == ROLE_SimulatedProxy)
	{
...
void AEcoscapeCharacter::PreReplication( IRepChangedPropertyTracker & ChangedPropertyTracker )
{
	...
	if (CharacterMovement->CurrentRootMotion.HasActiveRootMotionSources() || IsPlayingNetworkedRootMotionMontage())
	{
		...
		DOREPLIFETIME_ACTIVE_OVERRIDE( AEcoscapeCharacter, RepRootMotion, true );
	}
	else
	{
		RepRootMotion.Clear();

		DOREPLIFETIME_ACTIVE_OVERRIDE( AEcoscapeCharacter, RepRootMotion, false );
	}
...
void AEcoscapeCharacter::PreReplicationForReplay(IRepChangedPropertyTracker& ChangedPropertyTracker)
{
...
void AEcoscapeCharacter::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
	Super::GetLifetimeReplicatedProps( OutLifetimeProps );

	DISABLE_REPLICATED_PROPERTY(AEcoscapeCharacter, JumpMaxHoldTime);
	DISABLE_REPLICATED_PROPERTY(AEcoscapeCharacter, JumpMaxCount);

	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, RepRootMotion,						COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, ReplicatedBasedMovement,			COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, ReplicatedServerLastTransformUpdateTimeStamp, COND_SimulatedOnlyNoReplay );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, ReplicatedMovementMode,			COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, bIsCrouched,						COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, bProxyIsJumpForceApplied,			COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, AnimRootMotionTranslationScale,	COND_SimulatedOnly );
	DOREPLIFETIME_CONDITION( AEcoscapeCharacter, ReplayLastTransformUpdateTimeStamp, COND_ReplayOnly );
}

bool AEcoscapeCharacter::IsPlayingRootMotion() const
{
	if (Mesh)
	{
...
bool AEcoscapeCharacter::HasAnyRootMotion() const
{
	return CharacterMovement ? CharacterMovement->HasRootMotionSources() : false;
}

bool AEcoscapeCharacter::IsPlayingNetworkedRootMotionMontage() const
{
	if (Mesh)
	{
...
void AEcoscapeCharacter::SetAnimRootMotionTranslationScale(float InAnimRootMotionTranslationScale)
{
	AnimRootMotionTranslationScale = InAnimRootMotionTranslationScale;
}

float AEcoscapeCharacter::GetAnimRootMotionTranslationScale() const
{
	return AnimRootMotionTranslationScale;
}

float AEcoscapeCharacter::PlayAnimMontage(class UAnimMontage* AnimMontage, float InPlayRate, FName StartSectionName)
{
...
void AEcoscapeCharacter::StopAnimMontage(class UAnimMontage* AnimMontage)
{
	UAnimInstance * AnimInstance = (Mesh)? Mesh->GetAnimInstance() : nullptr; 
...
class UAnimMontage * AEcoscapeCharacter::GetCurrentMontage() const
{
	UAnimInstance * AnimInstance = (Mesh)? Mesh->GetAnimInstance() : nullptr; 
	if ( AnimInstance )
	{
...
void AEcoscapeCharacter::ClientCheatWalk_Implementation()
{
...
void AEcoscapeCharacter::ClientCheatFly_Implementation()
{
#if !UE_BUILD_SHIPPING
	SetActorEnableCollision(true);
	if (CharacterMovement)
...
void AEcoscapeCharacter::ClientCheatGhost_Implementation()
{
#if !UE_BUILD_SHIPPING
...
void AEcoscapeCharacter::RootMotionDebugClientPrintOnScreen_Implementation(const FString& InString)
{
...
// ServerMovePacked
void AEcoscapeCharacter::ServerMovePacked_Implementation(const FCharacterServerMovePackedBits& PackedBits)
{
	GetCharacterMovement()->ServerMovePacked_ServerReceive(PackedBits);
}

bool AEcoscapeCharacter::ServerMovePacked_Validate(const FCharacterServerMovePackedBits& PackedBits)
{
...
// ServerMove
void AEcoscapeCharacter::ServerMove_Implementation(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
bool AEcoscapeCharacter::ServerMove_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
// ServerMoveNoBase
void AEcoscapeCharacter::ServerMoveNoBase_Implementation(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, uint8 ClientMovementMode)
{
...
bool AEcoscapeCharacter::ServerMoveNoBase_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, uint8 ClientMovementMode)
{
...
void AEcoscapeCharacter::ServerMoveDualNoBase_Implementation(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, uint8 ClientMovementMode)
{
...
bool AEcoscapeCharacter::ServerMoveDualNoBase_Validate(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, uint8 ClientMovementMode)
{
...
void AEcoscapeCharacter::ServerMoveDualHybridRootMotion_Implementation(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
// ServerMoveOld
void AEcoscapeCharacter::ServerMoveOld_Implementation(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags)
{
	GetCharacterMovement()->ServerMoveOld_Implementation(OldTimeStamp, OldAccel, OldMoveFlags);
}

bool AEcoscapeCharacter::ServerMoveOld_Validate(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags)
{
	return GetCharacterMovement()->ServerMoveOld_Validate(OldTimeStamp, OldAccel, OldMoveFlags);
}

// ClientMoveResponsePacked
void AEcoscapeCharacter::ClientMoveResponsePacked_Implementation(const FCharacterMoveResponsePackedBits& PackedBits)
{
	GetCharacterMovement()->MoveResponsePacked_ClientReceive(PackedBits);
}

bool AEcoscapeCharacter::ClientMoveResponsePacked_Validate(const FCharacterMoveResponsePackedBits& PackedBits)
{
...
// ClientAckGoodMove
void AEcoscapeCharacter::ClientAckGoodMove_Implementation(float TimeStamp)
{
	GetCharacterMovement()->ClientAckGoodMove_Implementation(TimeStamp);
}

// ClientAdjustPosition
void AEcoscapeCharacter::ClientAdjustPosition_Implementation(float TimeStamp, FVector NewLoc, FVector NewVel, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void AEcoscapeCharacter::ClientVeryShortAdjustPosition_Implementation(float TimeStamp, FVector NewLoc, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void AEcoscapeCharacter::ClientAdjustRootMotionPosition_Implementation(float TimeStamp, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void AEcoscapeCharacter::ClientAdjustRootMotionSourcePosition_Implementation(float TimeStamp, FRootMotionSourceGroup ServerRootMotion, bool bHasAnimRootMotion, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void AEcoscapeCharacter::FillAsyncInput(FCharacterAsyncInput& Input) const
{
...
void AEcoscapeCharacter::InitializeAsyncOutput(FCharacterAsyncOutput& Output) const
{
...
void AEcoscapeCharacter::ApplyAsyncOutput(const FCharacterAsyncOutput& Output)
{
