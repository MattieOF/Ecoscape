...
#include "EcoscapeCharacter.generated.h"
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FEcoscapeMovementModeChangedSignature, class AEcoscapeCharacter*, Character, EMovementMode, PrevMovementMode, uint8, PreviousCustomMode);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FEcoscapeCharacterMovementUpdatedSignature, float, DeltaSeconds, FVector, OldLocation, FVector, OldVelocity);
DECLARE_DYNAMIC_MULTICAST_DELEGATE(FEcoscapeCharacterReachedApexSignature);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FEcoscapeLandedSignature, const FHitResult&, Hit);

/** Replicated data when playing a root motion montage. */
USTRUCT()
struct FEcoscapeRepRootMotionMontage
{
...
USTRUCT()
struct FEcoscapeSimulatedRootMotionReplicatedMove
{
	GENERATED_USTRUCT_BODY()
	...
	UPROPERTY()
	FEcoscapeRepRootMotionMontage RootMotion;
};
...
namespace EcoscapeMovementBaseUtility
{
	...
	ECOSCAPE_API bool IsDynamicBase(const UPrimitiveComponent* MovementBase);

	/** Determine whether MovementBase is simulating or attached to a simulating object. */
	ECOSCAPE_API bool IsSimulatedBase(const UPrimitiveComponent* MovementBase);
	...
	ECOSCAPE_API void AddTickDependency(FTickFunction& BasedObjectTick, UPrimitiveComponent* NewBase);

	/** Remove tick dependency of BasedObjectTick on OldBase */
	ECOSCAPE_API void RemoveTickDependency(FTickFunction& BasedObjectTick, UPrimitiveComponent* OldBase);
	...
	ECOSCAPE_API FVector GetMovementBaseVelocity(const UPrimitiveComponent* MovementBase, const FName BoneName);

	/** Get the tangential velocity at WorldLocation for the given component. */
	ECOSCAPE_API FVector GetMovementBaseTangentialVelocity(const UPrimitiveComponent* MovementBase, const FName BoneName, const FVector& WorldLocation);
	...
	ECOSCAPE_API bool GetMovementBaseTransform(const UPrimitiveComponent* MovementBase, const FName BoneName, FVector& OutLocation, FQuat& OutQuat);
}

/** Struct to hold information about the "base" object the character is standing on. */
USTRUCT()
struct FEcoscapeBasedMovementInfo
{
	GENERATED_USTRUCT_BODY()
	...
	{
		return EcoscapeMovementBaseUtility::UseRelativeLocation(MovementBase);
	}
...
class ECOSCAPE_API AEcoscapeCharacter : public APawn
{
	GENERATED_BODY()
public:
	/** Default UObject constructor. */
	AEcoscapeCharacter(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
protected:
	...
	UPROPERTY()
	struct FEcoscapeBasedMovementInfo BasedMovement;
	...
	struct FEcoscapeBasedMovementInfo ReplicatedBasedMovement;
...
public:
	...
	FORCEINLINE const FEcoscapeBasedMovementInfo& GetBasedMovement() const { return BasedMovement; }
	
	/** Accessor for ReplicatedBasedMovement */
	FORCEINLINE const FEcoscapeBasedMovementInfo& GetReplicatedBasedMovement() const { return ReplicatedBasedMovement; }
...
public:
	...
	FEcoscapeCharacterReachedApexSignature OnReachedJumpApex;
	 ...
	 * @see OnMovementModeChanged()
	 */
	FEcoscapeLandedSignature LandedDelegate;
	...
	UPROPERTY(BlueprintAssignable, Category=Character)
	FEcoscapeMovementModeChangedSignature MovementModeChangedDelegate;
	 ...
	 */
	UPROPERTY(BlueprintAssignable, Category=Character)
	FEcoscapeCharacterMovementUpdatedSignature OnCharacterMovementUpdated;
	...
	UPROPERTY(Transient)
	TArray<FEcoscapeSimulatedRootMotionReplicatedMove> RootMotionRepMoves;
	...
	bool CanUseRootMotionRepMove(const FEcoscapeSimulatedRootMotionReplicatedMove& RootMotionRepMove, const FAnimMontageInstance& ClientMontageInstance) const;

	/** Restore actor to an old buffered move. */
	bool RestoreReplicatedMove(const FEcoscapeSimulatedRootMotionReplicatedMove& RootMotionRepMove);
	...
	struct FEcoscapeRepRootMotionMontage RepRootMotion;
